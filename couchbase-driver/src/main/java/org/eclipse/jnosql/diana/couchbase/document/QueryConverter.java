...
final class QueryConverter {
    ...
    static QueryConverterResult select(DocumentQuery query, String bucket) {
        ...
        if (query.getCondition().isPresent()) {
            ...
            if (nonNull(condition)) {
                statement = StatementFactory.create(bucket, documents, skip, limit, sorts, condition);
            } else {
                statement = null;
            }
        } else {
            statement = StatementFactory.create(bucket, documents, skip, limit, sorts);
        }
    ...
    private static void eqKeys(List<String> keys, String documentCollection, Document document) {
        if(document.getName().equals(EntityConverter.KEY_FIELD)){
    ...
    private static boolean isKeyField(Document document) {
        return EntityConverter.ID_FIELD.equals(document.getName()) || EntityConverter.KEY_FIELD.equals(document.getName());
    }
