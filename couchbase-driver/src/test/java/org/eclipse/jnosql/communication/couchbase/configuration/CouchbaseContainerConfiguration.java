...
public class CouchbaseContainerConfiguration {
    ...
    private void createBucket() {
        ...
        BucketSettings bucketSettings = DefaultBucketSettings.builder()
                .enableFlush(true).name(CouchbaseUtil.BUCKET_NAME).quota(100).replicas(0)
                .type(BucketType.COUCHBASE).build();
        clusterManager.insertBucket(bucketSettings);
        Bucket bucket = cluster.openBucket(CouchbaseUtil.BUCKET_NAME);
        bucket.query(N1qlQuery.simple("CREATE PRIMARY INDEX index_" + CouchbaseUtil.BUCKET_NAME + " on " + CouchbaseUtil.BUCKET_NAME));
        bucket.close();
    }
